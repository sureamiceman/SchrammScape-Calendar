...
struct SchrammScape_CalendarApp: App {
    ...
    init() {
        do {
            container = try ModelContainer(for: Customer.self, WorkRecord.self, ServiceItem.self, JobType.self, Invoice.self, MileageEntry.self)
        } catch {
            fatalError("Could not create ModelContainer: \(error)")
        }
        JobAlertService.shared.setUp(container: container)
        SyncEngine.shared.setUp(container: container)
    }
